...
public class GetLeaveAllocationListHandler : IRequestHandler<GetLeaveAllocationListQuery, List<LeaveAllocationDto>>
{
    ...
    {
        List<Domain.Entities.LeaveAllocation> leaveAllocations = await _leaveAllocationRepository.GetLeaveAllocationsWithDetails();
        List<LeaveAllocationDto> allocations = _mapper.Map<List<LeaveAllocationDto>>(leaveAllocations);
